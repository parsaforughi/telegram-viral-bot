...
import { categoryKeyboard, cleanserSubmenu, creamSubmenu, languageKeyboard, formatPostMessage, texts, viewKeyboard } from './messages.js';
import { getUserState, recordUserResults, upsertUserState } from './state.js';
const DEFAULT_LANGUAGE = 'en';
const clearInlineKeyboard = async (ctx) => {
    try {
...
const enNum = (x) => new Intl.NumberFormat('en-US').format(x);
export const buildBot = (token) => {
    if (!token) {
    ...
    bot.action(/^view_(.+)$/, async (ctx) => {
        ...
        const progressUpdater = (async () => {
            for (const stage of progressStages) {
                ...
                try {
                    await ctx.telegram.editMessageText(progressChatId, progressMessageId, undefined, stage);
                }
                catch {
        ...
        try {
            await ctx.telegram.editMessageText(progressChatId, progressMessageId, undefined, '✔ نتایج آماده شد!');
        }
        catch {
            // ignore
        }
        ...
        if (initialSent < totalResults) {
            const promptText = `📦 تا الان ${enNum(initialSent)} تا از ${enNum(totalResults)} پست رو برات فرستادم.\nادامه بدم؟ 🔎`;
    ...
    bot.action('next_batch', async (ctx) => {
        ...
        if (newOffset < results.length) {
            const promptText = `📦 تا الان ${enNum(newSent)} تا از ${enNum(total)} پست رو برات فرستادم.\nادامه بدم؟ 🔎`;
            await ctx.reply(promptText, continueKeyboard);
        }
        else {
            await ctx.reply('تمام شد! ✔️');
    ...
    bot.action('stop', async (ctx) => {
        ...
        await ctx.reply(`${firstName}، امیدوارم چندتا ایدهٔ خوب گرفته باشی.\nهر وقت خواستی دوباره سراغ وایرال‌ها بری، من آماده‌ام. ⚡️`);
    });
    return bot;
};
